...
raigal produces a single deterministic score from a scan (see `docs/scoring.md`).
...
1. **Trigger.** A user starts a fix run from one of:
   ...
   - the CLI (`raigal fix --open-pr`, which calls the hosted API).
...
repeat up to N iterations:
    raigal fix                      # mechanical fixers (deterministic)
    if findings remain:
...
- `raigal fix` runs first each iteration because it is deterministic and free; the
...
`raigal/fix/<id>` where `<id>` is the job id (short, URL-safe). One branch per run,
...
2. Push the iteration commits to `raigal/fix/<id>`.
3. `pulls.create({ base, head: "raigal/fix/<id>", title, body })`.
...
- Footer: raigal version + config hash so the result is reproducible.
...
- The App identity is the PR author, so changes are clearly attributable to raigal
...
- **Command allowlist.** The loop may run only `git`, `raigal`, and the project's
...
- **Score-gated writes.** Nothing is committed unless the deterministic score improved;
  ...
  the job).
- **No config tampering.** `.raigal/config.yaml` and `.raigal/rules.yaml` are treated
...
- **Worker** runs the sandboxed loop and shells out to the same `raigal` CLI used
