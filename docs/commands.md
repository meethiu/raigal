...
Use `raigal --help` for the short overview, `raigal commands` for the full public command list, and `raigal <command> --help` for command-specific help. (`aislop` is accepted as a command alias for backwards compatibility).
