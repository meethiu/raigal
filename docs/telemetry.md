...
`raigal` collects anonymous usage analytics to help prioritize improvements. **No code, file paths, project names, repo names, branch names, raw diagnostics, or secrets are ever collected.**
...
| Event | When |
|---|---|
| `cli_installed` | First-ever run on a machine (when `~/.raigal/install_id` is created) |
...
| `mcp_server_started` | After the `raigal-mcp` stdio transport connects |
| `mcp_tool_called` | Each `raigal_scan` / `raigal_fix` / `raigal_why` / `raigal_baseline` invocation |
| `hook_scan_completed` | After a Claude / Cursor / Gemini / pi agent hook finishes a scoped scan |

Each event carries:

- `raigal_version`, `node_version`, `os`, `arch`
- `schema_version` (currently `"v2"`)
- `anonymous_install_id` — random UUID stored in `~/.raigal/install_id` (re-rolls if you delete the file)
- `package_manager` — install channel: `npm` / `pnpm` / `yarn` / `bun` / `npx` / `homebrew` / `pip` / `pipx` / `direct` / `unknown` (Python and other shims may set `RAIGAL_INSTALL_CHANNEL` before invoking the Node CLI)
...
Telemetry uses a random UUID stored at `~/.raigal/install_id` (or `$XDG_STATE_HOME/raigal/install_id` on Linux). Deleting the file re-rolls your identity. The file is created with `0600` permissions and is never created if telemetry is disabled.

## Opt out

Precedence (highest wins):

1. `RAIGAL_NO_TELEMETRY=1` or `DO_NOT_TRACK=1` environment variables → off, always.
2. `.raigal/config.yml` → `telemetry.enabled: false` → off.
3. `.raigal/config.yml` → `telemetry.enabled: true` → on (this also overrides the CI default).
4. `CI=true` with no explicit config → off.
5. Default → on.

```bash
# Environment variable (any of these)
RAIGAL_NO_TELEMETRY=1 raigal scan
DO_NOT_TRACK=1 raigal scan
```
...
Set `RAIGAL_TELEMETRY_DEBUG=1` to print every outgoing event to stderr as JSON. Combine with `RAIGAL_TELEMETRY_DRY_RUN=1` to print without sending — useful for "what would this command emit?"

```bash
RAIGAL_TELEMETRY_DEBUG=1 RAIGAL_TELEMETRY_DRY_RUN=1 raigal scan
```
