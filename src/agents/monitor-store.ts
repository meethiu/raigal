...
const agentMonitorDir = (root: string): string => {
	const raigalDir = path.join(root, ".raigal", "agent", "monitors");
	if (fs.existsSync(raigalDir)) return raigalDir;
	const legacyDir = path.join(root, ".aislop", "agent", "monitors");
