...
const readGitState = async (cwd: string): Promise<GitState> => {
	...
	if (root.exitCode !== 0 || !root.stdout) {
		throw new Error("raigal agent needs to run inside a git repository.");
	}
...
const ensureLocalRaigalExclude = (gitCommonDir: string): void => {
	...
	const entries = [
		...
		".raigal/agent/logs/",
		".raigal/agent/monitors/",
		".raigal/agent/provider.json",
		".aislop/worktrees/",
	...
	fs.appendFileSync(
		excludePath,
		`${prefix}# raigal local agent sessions and worktrees\n${missingEntries.join("\n")}\n`,
		"utf-8",
	);
};
...
export const prepareAgentLocalState = async (cwd: string): Promise<{ root: string }> => {
	...
	ensureLocalRaigalExclude(state.gitCommonDir);
...
export const createAgentWorktree = async (cwd: string, opts: { inPlace: boolean }) => {
	...
	if (state.dirty) {
		throw new Error(
			"Current worktree has uncommitted changes. Commit/stash them, or rerun with --in-place if you want raigal agent to edit here.",
		);
	}
	const name = `agent-${timestamp()}-${process.pid}`;
	const worktreeRoot = path.join(state.root, ".raigal", "worktrees");
