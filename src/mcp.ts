...
export const buildServer = (): McpServer => {
	...
	server.registerTool(
		...
		(input) => instrument("raigal_scan", () => handleAislopScan(input)),
	);
	...
	server.registerTool(
		...
		(input) => instrument("raigal_fix", () => handleAislopFix(input)),
	);
	...
	server.registerTool(
		...
		(input) => instrument("raigal_why", () => handleAislopWhy(input)),
	);
	...
	server.registerTool(
		...
		(input) => instrument("raigal_baseline", () => handleAislopBaseline(input)),
	);
...
main().catch((e) => {
	process.stderr.write(
		`raigal-mcp failed to start: ${e instanceof Error ? e.message : String(e)}\n`,
	);
	process.exit(1);
});
