...
const providerStatusItem = (status: ProviderStatus): DisplayStatusItem => {
	...
	rows.push({ label: "Connect", value: `raigal agent connect ${status.provider.id}` });
...
export const renderAgentProviders = (
	...
	lines.push(
		...
		...renderDisplayCommandRows(
			[
				{ label: "Switch", command: `raigal agent --provider <${providerIds().join("|")}|auto>` },
				{ label: "Save", command: "raigal agent use <provider|auto>" },
			],
			{ indent: 3 },
		),
	);
