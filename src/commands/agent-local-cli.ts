...
export const scanJson = (cwd: string): AgentScanJson => {
	...
	if (!result.stdout.trim()) {
		throw new Error(result.stderr || "raigal scan did not produce JSON output.");
	}
	try {
		return JSON.parse(result.stdout) as AgentScanJson;
	} catch (error) {
		throw new Error(
			`Failed to parse raigal scan output: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
};
