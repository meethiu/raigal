...
export const hookRun = async (
	...
	if (process.stdin.isTTY) {
		process.stderr.write(
			`raigal hook ${agent} is an internal callback the agent invokes automatically. It reads a payload on stdin and has nothing to do interactively.\n\nYou probably want:\n  raigal hook install --${agent}     (install the hook for ${agent})\n  raigal hook status                   (see what's installed)\n  raigal hook uninstall --${agent}   (remove it)\n`,
		);
		process.exit(0);
	}
	let exitCode = 0;
...
export const promptAgentSelection = async (
	...
	const choice = await searchMultiselect<AgentName>({
		message:
			mode === "install"
				? "Which agents should get Raigal hooks?"
