...
const skipUnsafeSafeFormatter = (deps: PipelineDeps, language: "ruby" | "php"): boolean => {
	...
	log.warn(
		`Safe mode skips ${label} formatting because ${tool} can execute project-controlled configuration. Run \`raigal fix\` without --safe if you trust this repository.`,
	);
	return true;
};
...
// than run over code the user asked Raigal to leave alone.
