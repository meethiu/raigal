...
const ENGINE_PRESENTATION: Record<string, { label: string; summary: string; order: number }> = {
	...
	format: {
		label: "Format",
		summary: "Formatter and import-order checks that Raigal can usually fix.",
		order: 40,
	},
	lint: {
		label: "Lint",
	...
	architecture: {
		label: "Architecture",
		summary: "Project-specific import and layering rules from .raigal/rules.yml.",
		order: 60,
	},
};
...
export const buildRulesRender = (input: BuildRulesRenderInput): string => {
	...
	const lines: string[] = [
		` ${style(theme, "muted", "auto = raigal fix can change it; review = inspect and fix with a developer or agent.")}`,
...
export const buildRuleDetailRender = (
	...
	const rows = [
		...
		[
			"Fix",
			`${fixModeLabel(rule.fixable)}${rule.fixable ? " (raigal fix can change it)" : " (review and fix intentionally)"}`,
		],
	...
	const tail = renderHintLine(
		rule.fixable
			? "Run raigal fix to apply the automatic fix"
