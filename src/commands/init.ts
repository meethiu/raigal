...
const promptForConfigChoices = async (): Promise<InitChoices | null> => {
	...
	const telemetryChoice = await select<"enabled" | "disabled">({
		message: "Share anonymous usage stats so Raigal can improve?",
		options: [
	...
	const workflowChoice = await select<"yes" | "no">({
		message: "Add a GitHub Actions workflow to run Raigal on every push and PR?",
		options: [
			{ value: "yes", label: "Yes — writes .github/workflows/raigal.yml" },
