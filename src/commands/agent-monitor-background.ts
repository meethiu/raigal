...
export const launchMonitorInBackground = async (
	...
	const logDir = path.join(root, ".raigal", "agent", "logs");
...
export const renderMonitorBackgroundLaunch = (launch: MonitorBackgroundLaunch): void => {
	process.stdout.write(
		`${[
			...
			...renderDisplayCommandRows([
				{ label: "Inspect", command: `raigal agent monitor show ${launch.monitorId}` },
				{ label: "List", command: "raigal agent monitor list" },
			]),
			"",
		].join("\n")}`,
	);
