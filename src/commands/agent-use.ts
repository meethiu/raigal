...
const renderCurrentPreference = (root: string): string => {
	...
	if (!preference) {
		return [
			...
			...renderDisplayCommandRows([
				{ label: "Set", command: "raigal agent use codex|claude|opencode" },
			]),
			"",
		].join("\n");
	}
...
const warnIfProviderNeedsSetup = (provider: AgentProviderId): void => {
	...
	if (!status.installed) {
		...
		log.muted(`Install ${status.provider.bin}, then run \`raigal agent connect ${provider}\`.`);
		return;
	}
...
export const agentUseCommand = async (
	...
	try {
		...
		log.muted("Override once with `raigal agent --provider <provider>`.");
	} catch (error) {
