...
export const renderAgentMonitorList = (input: {
	...
	if (input.monitors.length === 0) {
		...
		lines.push(
			...
			...renderDisplayCommandRows([
				{ label: "Start", command: "raigal agent monitor --background" },
			]),
		);
		return `${lines.join("\n")}\n`;
	}
	...
	lines.push(
		...
		renderDisplaySection("Next"),
		...renderDisplayCommandRows([
			{ label: "Details", command: "raigal agent monitor show <monitor>" },
		]),
	);
	return `${lines.join("\n")}\n`;
};
	...
	try {
		...
		process.stdout.write(
			`${[
				...
				renderDisplaySection("Next"),
				...renderDisplayCommandRows([
					{ label: "Inspect", command: `raigal agent monitor show ${stopped.id}` },
				]),
				"",
			].join("\n")}`,
		);
