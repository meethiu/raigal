...
const guideNoProvider = (statuses: ProviderStatus[]): void => {
	...
	log.error(
		anyInstalled
			...
			: "No coding agent is installed. `raigal agent` drives Codex, Claude Code, or OpenCode.",
	);
	...
	log.muted("Set one up, then re-run. `raigal agent connect` walks through it too.");
};
