...
export const isUpdateNotifierDisabled = (env: NodeJS.ProcessEnv = process.env): boolean => {
	if (env.RAIGAL_NO_UPDATE_NOTIFIER === "1" || env.AISLOP_NO_UPDATE_NOTIFIER === "1") return true;
...
export const resolveUpdateCachePath = (
	...
	if (process.platform === "linux" && env.XDG_STATE_HOME) {
		return path.join(env.XDG_STATE_HOME, "raigal", CACHE_BASENAME);
	}
	return path.join(homedir, ".raigal", CACHE_BASENAME);
};
