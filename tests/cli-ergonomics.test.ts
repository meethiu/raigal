...
describe("cli ergonomics", () => {
	it("uses the installed command in top-level help and keeps npx for one-off latest runs", () => {
		...
		expect(result.stdout).toMatch(/raigal ci|aislop ci/);
		expect(result.stdout).toMatch(/raigal agent|aislop agent/);
		expect(result.stdout).toContain("--safe");
		expect(result.stdout).toMatch(/\.raigalignore|\.aislopignore/);
		expect(result.stdout).toMatch(/raigal commands|aislop commands/);
		expect(result.stdout).toMatch(/raigal hook install --claude|aislop hook install --claude/);
		expect(result.stdout).toMatch(/raigal install hooks|aislop install hooks/);
		expect(result.stdout).toMatch(/npx (raigal|aislop)@latest scan/);
	...
	it("lists all commands and major flags", () => {
		...
		expect(result.stdout).toMatch(/(raigal|aislop) \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) ci \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent plan \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent providers/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent connect \[provider\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent switch \[provider\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent sessions \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent show \[session\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent apply \[session\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent watch \[session\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent monitor \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent monitor list \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent monitor show \[monitor\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent monitor stop \[monitor\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) agent stop \[session\]/);
		...
		expect(result.stdout).toMatch(/(raigal|aislop) hooks/);
		expect(result.stdout).toMatch(/(raigal|aislop) hook/);
		expect(result.stdout).toMatch(/(raigal|aislop) hook uninstall/);
		expect(result.stdout).toMatch(/(raigal|aislop) hook baseline/);
		expect(result.stdout).toMatch(/(raigal|aislop) install \[agents\.\.\.\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) uninstall \[agents\.\.\.\]/);
		...
		expect(result.stdout).toMatch(/(raigal|aislop) badge \[directory\]/);
		expect(result.stdout).toMatch(/(raigal|aislop) trends \[directory\]/);
		...
		expect(result.stdout).toMatch(/(raigal|aislop) <command> --help/);
	...
	it("exposes local agent provider status and repair session help", () => {
		...
		expect(providers.stdout).toMatch(/(raigal|aislop) agent connect codex/);
		expect(providers.stdout).toMatch(/(raigal|aislop) agent use <provider\|auto>/);
	...
	it("exposes a local default-provider switch without provider keys", () => {
		const tempGit = mkdtempSync(path.join(tmpdir(), "aislop-git-"));
		spawnSync("git", ["init", "-b", "main"], { cwd: tempGit });
		try {
			const result = runCli(["agent", "use", "codex", "--dry-run"], tempGit);

			expect(result.status).toBe(0);
			expect(result.stdout).toContain("Agent provider");
			expect(result.stdout).toContain("Dry run");
			expect(result.stdout).toMatch(/Provider\s+Codex \(codex\)/);
			expect(result.stdout).toMatch(/\.(raigal|aislop)\/agent\/provider\.json/);
		} finally {
	...
	it("routes hooks as an alias for the hook command", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stdout).toMatch(/Usage:\s+(raigal|aislop) hook/);
	...
	it("supports the natural install hooks command form", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stdout).toMatch(/Usage:\s+(raigal|aislop) install \[options\] \[agents\.\.\.\]/);
