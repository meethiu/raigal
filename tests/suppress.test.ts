...
import { applySuppressions, isRaigalDirectiveLine } from "../src/utils/suppress.js";

let tmpDir: string;

beforeEach(() => {
	tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "raigal-suppress-"));
});
...
describe("applySuppressions", () => {
	it("suppresses the next line for a bare directive", () => {
		write("a.ts", "// raigal-ignore-next-line\nconst x = {} || {};\nconst y = 1;\n");
	...
	it("suppresses the next line for a JSDoc-style block directive", () => {
		...
		expect(suppressedCount).toBe(1);
		expect(isRaigalDirectiveLine("/** raigal-ignore-next-line */")).toBe(true);
	});

	it("only suppresses the named rule when one is given", () => {
		write("a.ts", "// raigal-ignore-next-line ai-slop/empty-fallback\nconst x = 1;\n");
		const { results, suppressedCount } = applySuppressions(
	...
	it("matches a bare rule name against the engine-prefixed rule", () => {
		write("a.ts", "// raigal-ignore-next-line empty-fallback\nconst x = 1;\n");
		const { suppressedCount } = applySuppressions(
			wrap([diag("a.ts", 2, "ai-slop/empty-fallback")]),
			tmpDir,
		);
	...
	it("ignores a reason after the -- separator", () => {
		write(
			"a.ts",
			"// raigal-ignore-next-line ai-slop/empty-fallback -- intentional guard\nconst x = 1;\n",
		);
		const { suppressedCount } = applySuppressions(
			wrap([diag("a.ts", 2, "ai-slop/empty-fallback")]),
			tmpDir,
	...
	it("suppresses on the same line via raigal-ignore-line", () => {
		write("a.ts", "const x = {} || {}; // raigal-ignore-line\n");
		const { suppressedCount } = applySuppressions(
	...
	it("suppresses a whole file with raigal-ignore-file", () => {
		write("a.ts", "// raigal-ignore-file ai-slop/empty-fallback\nconst a = 1;\nconst b = 2;\n");
		const { suppressedCount } = applySuppressions(
			wrap([
	...
	it("works with non-slash comment syntax (python hash)", () => {
		write("a.py", "# raigal-ignore-next-line\nx = 1\n");
		const { suppressedCount } = applySuppressions(
	...
	it("ignores the directive words inside a string literal (no comment marker)", () => {
		write("a.ts", 'const help = "pass raigal-ignore-file to skip a file";\nconst y = 2;\n');
		const { suppressedCount } = applySuppressions(
	...
	it("ignores directive-looking URL text inside string literals", () => {
		write("a.ts", 'const url = "https://raigal-ignore-file";\nconst y = 2;\n');
		const { results, suppressedCount } = applySuppressions(
			wrap([diag("a.ts", 1, "ai-slop/other"), diag("a.ts", 2, "ai-slop/other")]),
			tmpDir,
		);
		...
		expect(isRaigalDirectiveLine('const url = "https://raigal-ignore-file";')).toBe(false);
	});

	it("ignores directive-looking block and hash markers inside string literals", () => {
		write(
			"a.ts",
			'const block = "/* raigal-ignore-file";\nconst hash = "# raigal-ignore-next-line";\nconst y = 2;\n',
		);
		const { suppressedCount } = applySuppressions(
			wrap([
...
describe("raigal directive lines are invisible to comment blocks", () => {
	it("excludes a directive line so the comment below forms its own block", () => {
		...
		const lines = [
			"// raigal-ignore-next-line ai-slop/narrative-comment",
