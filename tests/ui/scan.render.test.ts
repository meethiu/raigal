...
describe("scan render", () => {
	it("includes header, summary with score, and next-steps when there are issues", () => {
		...
		expect(out).toMatch(/(raigal|aislop)/);
		expect(out).toContain("Scan result");
		expect(out).toContain("my-app");
		expect(out).toMatch(/89 \/ 100\s+Healthy/);
		expect(out).toContain("Agent repair plan");
		expect(out).toMatch(/Agent\s+(raigal|aislop) agent\s+run a local worktree repair session/);
		expect(out).toMatch(
			/Provider\s+(raigal|aislop) agent --provider codex\s+or use --provider claude\/opencode/,
		);
		expect(out).toMatch(
			/Preview\s+(raigal|aislop) agent plan\s+preview provider, worktree, findings, and publish actions/,
		);
		expect(out).toMatch(/Auto-fix\s+(raigal|aislop) fix\s+auto-fix 1 issue/);
		expect(out).not.toMatch(/(raigal|aislop) fix --claude/);
		expect(out).toContain("14-day team baseline");
		expect(out).toContain("https://raigal.dev/contact?intent=team-baseline");
	});
