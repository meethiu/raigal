...
describe("GitHub action manifest", () => {
	...
	it("exercises default, latest, and pinned action modes in CI", () => {
		const workflowPath = fs.existsSync(path.join(rootDir, ".github/workflows/raigal.yml"))
			? path.join(rootDir, ".github/workflows/raigal.yml")
			: path.join(rootDir, ".github/workflows/aislop.yml");
		const workflow = YAML.parse(fs.readFileSync(workflowPath, "utf-8")) as Workflow;
		...
		expect(pinnedStep?.with?.version).toBe("1.0.0");
	});
});
