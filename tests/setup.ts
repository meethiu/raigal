...
import {
    ...
    generateTestKeyPair, // assuming this was meant to be kept from your original snippet
    signTestEntitlementJwt,
    TEST_KID,
    TEST_PUBLIC_KEY_PEM,
} from "./cloud/helpers.js";

// --- 1. Custom Vitest Matchers ---
...
expect.extend({
    toContainPath(received: string, expected: string) {
        ...
        return {
            pass,
            message: () => `expected path "${received}" to ${pass ? "not " : ""}contain "${expected}"`,
        };
    },
});

// --- 2. Global Test Credentials Setup ---
