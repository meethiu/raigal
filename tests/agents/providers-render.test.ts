...
describe("agent providers render", () => {
	it("uses reusable sections and aligned rows", () => {
		...
		expect(out).toMatch(/Connect\s+(raigal|aislop) agent connect codex/);
		expect(out).toContain("Actions");
		expect(out).toMatch(/Switch\s+(raigal|aislop) agent --provider <codex\|claude\|opencode\|auto>/);
		expect(out).toMatch(/Save\s+(raigal|aislop) agent use <provider\|auto>/);
		...
		expect(statusLine?.indexOf("ready")).toBe(connectLine?.search(/(raigal|aislop) agent connect codex/));
	});
});
