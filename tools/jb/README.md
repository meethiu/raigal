...
This directory holds raigal's bundled ReSharper/JetBrains settings for the
`jb inspectcode` pass. The settings file is passed via `--settings` when
`resolveBundledJbSettings()` finds it, allowing raigal to layer controlled
...
- **InconsistentNaming suppression** (`DO_NOT_SHOW`): The `InconsistentNaming`
  ...
  raigal force-suppresses it via this settings file so the inspection count is
...
then exported and merged into `raigal.DotSettings`. SSR blobs are opaque enough
...
2. Export the Solution-team-shared settings and copy the resulting
   `/Default/PatternsAndTemplates/...` entries into `raigal.DotSettings`.
3. Verify each pattern actually fires: build a tiny C# fixture containing one
   violation of each, then run
   `jb inspectcode <fixture> --format=Xml --output=ssr.xml --caches-home=<temp> --settings=tools/jb/raigal.DotSettings`
