...
- **Public score badge in the README header (#46).** Shields-compatible SVG served from `badges.raigal.dev`, edge-cached. Drop one line into any README that opts in:
  ```markdown
  [![aislop](https://badges.raigal.dev/score/<owner>/<repo>.svg)](https://raigal.dev/<owner>/<repo>)
  ```
...
- **`README.md` covers hooks.** New "Install as a native hook" section under Usage with adapter + rules-only installer lists, quality-gate flow, and a link to [`/docs/hooks`](https://raigal.dev/docs/hooks). Quick-start and "Why aislop" blocks mention hooks. Sample-output version banner bumped to 0.6.1.
